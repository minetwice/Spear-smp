    ...
    private void shootPoisonProjectile(Player player, Location location, Vector direction) {
        ...
        new BukkitRunnable() {
            public void run() {
                ...
                Location projLoc = projectile.getLocation();
                player.getWorld().spawnParticle(Particle.SPELL_WITCH, projLoc, 6, 0.2, 0.2, 0.2, 0.1);
                player.getWorld().spawnParticle(Particle.SMOKE_NORMAL, projLoc, 3, 0.3, 0.3, 0.3, 0.1);
            }
        }.runTaskTimer(MagicalSpearsPlugin.getInstance(), 0L, 1L);
